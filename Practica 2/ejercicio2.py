s = """NumPy is the fundamental package needed for scientific computing with Python.
...
#Recorro la lista y creo un diccionario con las palabras y
#contabilizando cuantas veces aparece en el texto
for word in words:
    if word in dicci.keys():
        dicci[word] = dicci[word] + 1
    else:
        dicci[word] = 1

#Recorro el diccionario para evaluar cual es la palabra
#que mas veces aparece
max = -999
for clave in dicci:
    if max < dicci[clave]:
        max = dicci[clave]
...
print(f"La palabra que aparecio mas veces es: {maxWord}")
print(dicci[maxWord])